...
const Product = (props) => {
    ...
    return (
        <div className="bg-white shadow flex flex-col items-center gap-2 px-2 py-6 relative" style={{minHeight:"360px"}}>
            {/* <!-- image & product title --> */}
            <Link to={`/product/${_id}`} className="flex flex-col items-center text-center group">
                ...
                <h2 className="text-sm mt-4 font-medium group-hover:text-primary-blue">{name.length > 50 ? `${name.substring(0, 50)}...` : name}</h2>
            </Link>
            {/* <!-- image & product title --> */}
            ...
            {/* <!-- wishlist badge --> */}
            <span onClick={addToWishlistHandler} className={`${itemInWishlist ? "text-red-500" : "hover:text-red-500 text-gray-300"} absolute top-5 right-2 cursor-pointer`}><FavoriteIcon sx={{ fontSize: "20px" }} /></span>
            {/* <!-- wishlist badge --> */}
